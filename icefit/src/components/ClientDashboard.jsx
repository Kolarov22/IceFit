...
const ClientDashboard = () => {
    ...
    const [activeTrainingPlans, setActiveTrainingPlans] = useState([]);
      ...
      const fetchTrainingPlans = async () => {
        try {
          const response = await fetch(
            "http://localhost:8080/v1/training/plans"
          );
        ...
        <div className="h-32">
          {activeTrainingPlans.map((plan) => (
            <div key={plan.id} className="p-2 border-b">
              ...
              <p className="text-sm">Instructor: {plan.instructor.username}</p>
            </div>
          ))}
        </div>
