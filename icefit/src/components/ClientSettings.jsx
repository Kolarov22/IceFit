...
const ClientSettings = () => {
  ...
  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      {message.text && (
        <div className={`fixed top-5 right-5 p-4 rounded-lg shadow-lg ${
      ...
      <div className="bg-white shadow-xl rounded-lg overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-8">
          ...
          <div className="bg-gray-50 p-6 rounded-lg shadow-md transform transition-all hover:shadow-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {Object.entries(notifications).map(([key, value]) => (
                <label key={key} className="flex items-center gap-3 p-3 bg-white rounded-lg shadow-sm hover:shadow-md transition-all cursor-pointer">
                  <input
                    type="checkbox"
                    checked={value}
                    onChange={(e) => {
                      setNotifications({...notifications, [key]: e.target.checked});
                      showMessage('success', `${key} notifications ${e.target.checked ? 'enabled' : 'disabled'}`);
                    }}
          ...
          <div className="bg-gray-50 p-6 rounded-lg shadow-md transform transition-all hover:shadow-lg">
            <h3 className="text-xl font-semibold mb-4 text-gray-800">Fitness Goals</h3>
            ...
            <ul className="space-y-3">
              {goals.map((goal, index) => (
                <li key={index} className="flex items-center gap-3 p-3 bg-white rounded-lg shadow-sm hover:shadow-md transition-all group">
                  <input
                    type="checkbox"
                    ...
                    onChange={() => {
                      ...
                      setGoals(newGoals);
                      showMessage('success', `Goal marked as ${newGoals[index].completed ? 'completed' : 'incomplete'}`);
                    }}
                    className="w-5 h-5 text-primary cursor-pointer"
                  />
                  <span className={`flex-1 transition-all ${goal.completed ? 'line-through text-gray-400' : 'text-gray-700'}`}>
                    {goal.text}
                  </span>
                  <button
                    type="button"
          ...
          <div className="flex justify-center gap-6">
            ...
            <button
              ...
              className={`px-8 py-3 bg-confirmGreen text-white rounded-lg hover:bg-green-600 transition-all transform hover:scale-105 active:scale-95 focus:ring-2 focus:ring-green-500 focus:ring-offset-2 flex items-center gap-2 ${isSubmitting ? 'opacity-75 cursor-not-allowed' : ''}`}
            >
              {isSubmitting ? (
                <>
